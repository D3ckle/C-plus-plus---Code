...
void bag(trainer player, POKEMON& p)
{
	...
	int item = stoi(str); // converts the string to int
...
void casino(trainer& player) //simulates the casino location for the game
{
	...
	do {
		...
		else if (betdb == 0) { // no bet, dont play the game
			turnend = true;
			break;
		}
	...
	if (betdb > 0) { // so long as there is a bet
		do {
			...
			if (playerpoint <= 21) { // wont execute if the player already lost, direct to lose text
				...
				cout << "Dealer's Total points: " << dealerpoint << endl << endl; // show their cards 
				system("pause");
				if (dealerpoint <= 17) { // prevents taking from deck if already above 17
					do {// dealer draw cards until over 17
						...
						cout << "Dealer's Total points: " << dealerpoint << endl << endl; // updated dealer's points
						system("pause");
			...
			cout << "You've lost / gained " << betdb << "$." << endl; // telling amount lsot or gained
			cout << "current $: " << player.getmoney() << endl; // tell player their money now
			system("pause");
			system("cls");
		} while (!turnend);
	}
...
int main() //runs the main logic of the code
{
	...
	do
	{
		...
		p.fullHeal();
		p.setLevel(9); //sets the level to 10 for testing purposes
		...
		do
		{
			...
			if (p.getTotalHP() == 0) //if tee current HP is 0, thats GAME OVER
				gameOver = true;
